...
const Game = {
  ...
  async showStatsGuide(flow) {
    ...
    // 卡片自动依次翻开（节奏放慢，让用户有时间阅读）
    await this.msgSleep(700);
    const cards = document.querySelectorAll('#statsPanel .stats-guide-cell');
    for (let i = 0; i < cards.length; i++) {
      cards[i].classList.add('revealed');
      if (i === 3) cards[i].classList.add('critical-pulse');
      await this.msgSleep(900);
    }

    // 翻完后显示「了解，继续」按钮
    const progress = document.getElementById('statsGuideProgress');
    if (progress) {
      progress.innerHTML = `<button class="stats-guide-continue-btn" id="statsGuideContinueBtn">了解，继续</button>`;
      document.getElementById('statsGuideContinueBtn').addEventListener('click', () => {
        // 关闭面板
        overlay.classList.remove('show');
        this.stopUserFluctuation();
        // 显示 fab（配提示）
  ...
  renderStatsGuidePanel() {
    ...
    panel.innerHTML = '<div class="stats-guide-intro-wrap">' +
      ...
      '</div>' +
      '<div class="stats-guide-progress" id="statsGuideProgress">数据加载中...</div>' +
  ...
  showStatsGuideComplete(flow) {
    ...
    tip.innerHTML = '你点了点头。弹药不多，但够用。<br><span style="font-size:0.85em;opacity:0.6">（随时点右下角 >_ 查看数据）</span>';
  ...
  restartGame(skipCommit) {
    ...
    const fab = document.getElementById('statsFab');
    if (fab) { fab.classList.remove('settle-hidden', 'visible'); fab.style.display = ''; }
  ...
  showEndScreen() {
    ...
    // === 保护性清理：确保所有 overlay 不遮挡结算页 ===
    const settleOv = document.getElementById('settleOverlay');
    if (settleOv) settleOv.classList.remove('show', 'fade-out');
    const panelOv = document.getElementById('statsPanelOverlay');
    if (panelOv) panelOv.classList.remove('show');
    const roundTr = document.getElementById('roundTransition');
    if (roundTr) roundTr.classList.remove('show');
    const whatsnewOv = document.getElementById('whatsnewOverlay');
    if (whatsnewOv) whatsnewOv.classList.remove('show');
    ...
    // === 先 commitRun 再渲染，确保 meta 包含本轮数据 ===
    const preCommitMeta = MetaSave.load();
    const preCommitAchs = [...preCommitMeta.achievements]; // 记录 commit 前的成就列表
    MetaSave.commitRun(this.state, title.name);
    const meta = MetaSave.load();

    // 累积成就（跨周目合集）和本轮新获得的成就
    const allAchs = meta.achievements;
    const newAchsThisRun = this.state.achievements.filter(a => !preCommitAchs.includes(a));
    const achCount = allAchs.length;
    ...
    container.innerHTML = `
      ...
      <div class="es-body">
        ...
        <div class="es-section">
          <div class="es-section-label">成就 <span class="es-ach-count">${achCount}/${this.ACH_REGISTRY.filter(a => a.round <= this._getMaxVisibleRound()).length}${newAchsThisRun.length > 0 ? ` <span class="es-ach-new-count">本轮 +${newAchsThisRun.length}</span>` : ''}</span></div>
          <div class="es-ach-list">
            ${achCount > 0 ? allAchs.map(name => {
              const info = this.findAchievementInfo(name);
              const isNew = newAchsThisRun.includes(name);
              return `<div class="es-ach-item${isNew ? ' es-ach-new' : ''}">
                <span class="es-ach-icon">${info.icon}</span>
                <div class="es-ach-info">
                  <div class="es-ach-name">${info.name}${isNew ? ' <span class="es-ach-new-tag">NEW</span>' : ''}</div>
          ...
          ${this._buildLockedAchHints(allAchs)}
        </div>
        <div class="es-section">
    ...
    if (shareBtn) {
      shareBtn.addEventListener('click', async () => {
        ...
        try {
          const imgData = await this._generateShareCard(title, s, init, playerName, avatarSrc, achCount, allAchs, newAchsThisRun);
          const preview = document.getElementById('screenshotPreview');
  ...
  async _generateShareCard(title, stats, init, playerName, avatarSrc, achCount, allAchs, newAchsThisRun) {
    const W = 750;
    const MAX_H = 2800;
    const canvas = document.createElement('canvas');
    ...
    const achList = (allAchs || this.state.achievements).map(name => {
      const info = this.findAchievementInfo(name);
      info._isNew = (newAchsThisRun || this.state.achievements).includes(name);
      return info;
    });
    ...
    const newCount = achList.filter(a => a._isNew).length;
    ...
    const achCountText = achCount + ' / ' + maxVisibleAch;
    const newText = newCount > 0 ? '  本轮 +' + newCount : '';
    ctx.fillText(achCountText, W - PAD, cy);
    if (newCount > 0) {
      const achW = ctx.measureText(achCountText).width;
      ctx.font = `600 18px ${SANS}`;
      ctx.fillStyle = 'rgba(255,200,0,0.7)';
      ctx.fillText(newText, W - PAD - achW - 4, cy);
    }
    ctx.textAlign = 'left';
    cy += 14;
    ...
    if (achList.length > 0) {
      const rowH = 64;
      achList.slice(0, 8).forEach((a, i) => {
        const ry = cy + i * rowH;
        if (i % 2 === 0) {
          rr(PAD, ry, CW, rowH, 6);
          ctx.fillStyle = a._isNew ? 'rgba(255,200,0,0.025)' : 'rgba(0,255,65,0.015)';
          ctx.fill();
        }
        ctx.textAlign = 'left';
        ...
        ctx.fillStyle = a._isNew ? 'rgba(255,200,0,0.9)' : GREEN + '0.85)';
        ...
        if (a._isNew) {
          ...
          ctx.fillText('NEW', PAD + 64 + nameW + 10, ry + 26);
        }
        ctx.font = `400 18px ${SANS}`;
      ...
      cy += Math.min(achList.length, 8) * rowH + 12;
    } else {
      rr(PAD, cy, CW, 56, 8);
    ...
    // 7.5 累计探索进度（跨周目）
    // ==============================
    const shareMeta = MetaSave.load();
    ctx.textAlign = 'left';
    ctx.font = `600 20px ${SANS}`;
    ctx.fillStyle = GREEN + '0.35)';
    ctx.fillText('> 累计探索进度', PAD, cy);
    cy += 28;

    const metaItems = [
      { label: '周目', val: String(shareMeta.totalRuns) },
      { label: '结局', val: String(shareMeta.endings.length) },
      { label: '成就', val: achCount + '/' + maxVisibleAch },
      { label: '档案', val: String(shareMeta.archivesViewed.length) },
    ];
    const metaColW = Math.floor(CW / 4);
    metaItems.forEach((m, i) => {
      const cx = PAD + i * metaColW + metaColW / 2;
      ctx.textAlign = 'center';
      ctx.font = `700 32px ${MONO}`;
      ctx.fillStyle = GREEN + '0.8)';
      ctx.fillText(m.val, cx, cy + 4);
    ...
    if (shareMeta.bestStats.maxUsers > 0 && shareMeta.totalRuns > 1) {
      ctx.font = `400 18px ${SANS}`;
      ctx.fillStyle = 'rgba(255,255,255,0.25)';
      ctx.textAlign = 'center';
      const fmtBest = (n) => {
      ...
      ctx.fillText('历史最佳：' + fmtBest(shareMeta.bestStats.maxUsers) + ' 用户 · ' + shareMeta.bestStats.maxReputation + '% 口碑', W / 2, cy);
      ctx.textAlign = 'left';
      cy += 28;
    }
  ...
  // 构建未解锁成就提示（基于累积成就判断）
  _buildLockedAchHints(allAchs) {
    const maxRound = this._getMaxVisibleRound();
    const chapterAchs = this.ACH_REGISTRY.filter(a => a.round <= maxRound);
    const unlockedNames = allAchs || this.state.achievements;
    const locked = chapterAchs.filter(a => !unlockedNames.includes(a.name));
  ...
  _buildMetaProgress() {
    ...
    html += `<div class="es-meta-title">> 累计探索进度${totalRuns <= 1 ? '（第1周目）' : ''}</div>`;
